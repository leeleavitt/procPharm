...
#####################################################
	...
	tmp.rd <- list(t.dat=t.dat,w.dat=w.dat,c.dat=c.dat, bin=bin, scp=scp, snr=pcp$snr, blc=pcp$blc, der=pcp$der, img1=img1, img2=img2, img3=img3, img4=img4)
#####################################################
# Cell Label Scoring	
#####################################################
	...
	if(pic.plot==TRUE & length(m.names)<=5){
		...
		for(i in 1:length(m.names)){
			...
			for(i in 1:length(m.names)){		
				...
				ytop<-pic.pos[[i]]+(.06*hb)
				ybottom<-pic.pos[[i]]-(.06*hb)
				rasterImage(img.p[top:bottom,left:right,],xleft,ytop,xright,ybottom)
			}
		}
	else{
		par(mar=c(0,0,0,0))
...
LinesStack.2.1 <- function(dat,m.names,lmain="",levs=NULL, plot.new=TRUE,bcex=.7, sf=.7, subset.n=5, img=NULL)
{
	...
	if(length(m.names)>subset.n){
		...
		for(i in 1:length(s.names)){
			...
			xright<-xleft+xinch(2.1)
			y<-(blc[nrow(t.dat),group.names[[i]]]+i*sf)
			ybottom<- y-yinch(.35)
			ytop<-y+yinch(.35)

			bp.img<-bpfunc.2(dat,group.names[[i]])
			dev.set(dev.current)
	...
	if(continue=="yes"){
		for(i in 1:length(s.names)){
			LinesEvery.4(dat,names(which(info==i, arr.ind=T)), img, pic.plot=T, sf=.3, lmain=i)
			#multi.pic.zoom(dat, names(which(info==i, arr.ind=T)), img, plot.new=T)
			menu(c("hello", "goodbye"))
		}
}
	}
	else{LinesEvery.4(dat, m.names,img)}
...
LinesStack.2.2 <- function(dat,m.names,lmain="",levs=NULL, plot.new=TRUE,bcex=.7, sf=.7, subset.n=5, img=NULL)
{
	if(is.null(img)){img<-dat$img.gtd}
	if(plot.new){dev.new(width=10,height=6)}
	if(length(m.names)>subset.n){
		...
		LinesEvery.4(dat, sample(row.names(dat$c.dat)[1:5]), plot.new=F)
	...
	if(continue=="yes"){
		while(i!=length(s.names)+1){
			i<-scan(n=1)
			LinesEvery.4(dat,sample(names(which(info==i, arr.ind=T)))[1:15], img, pic.plot=T, sf=.3, lmain=i)
			#multi.pic.zoom(dat, names(which(info==i, arr.ind=T)), img, plot.new=T)
		}
}
	}
...
PeakFunc6 <- function(dat,n.names,select.trace=F,Plotit.trace=T,Plotit.both=F, info=T,lmain=NULL, bcex=.7)
{
	...
	if(!is.null(dat$img.gtd)){
		xleft<-max(dat.t[,1])*1.05
		xright<-max(dat.t[,1])*1.13
		ytop<-ymax-(yrange*.05)
		ybottom<-ymax-(yrange*.45)
		rasterImage(dat$img.gtd[top:bottom,left:right,],xleft,ybottom,xright,ytop)
	}
	
	if(!is.null(dat$img.g)){
		xleft<-max(dat.t[,1])*1.15
		xright<-max(dat.t[,1])*1.23
		ytop<-ymax-(yrange*.05)
		ybottom<-ymax-(yrange*.45)
		rasterImage(dat$img.g[top:bottom,left:right,],xleft,ybottom,xright,ytop)
	}
	
	if(!is.null(dat$img.t)){
		xleft<-max(dat.t[,1])*1.05
		xright<-max(dat.t[,1])*1.13
		ytop<-ymax-(yrange*.55)
		ybottom<-ymax-(yrange*.95)
		rasterImage(dat$img.t[top:bottom,left:right,],xleft,ybottom,xright,ytop)
	}
	
	if(!is.null(dat$img.bl)){
		xleft<-max(dat.t[,1])*1.15
		xright<-max(dat.t[,1])*1.23
		ytop<-ymax-(yrange*.55)
		ybottom<-ymax-(yrange*.95)
		rasterImage(dat$img.bl[top:bottom,left:right,],xleft,ybottom,xright,ytop)
	}
}	

PeakFunc7 <- function(dat,n.names,select.trace=F,Plotit.trace=T,Plotit.both=F,plot.new=T, info=T,lmain=NULL, bcex=1, ylim.max=NULL)
{
	if(plot.new){dev.new(width=8, height=4)}
	...
	plot(dat.t[,n.names]~dat.t[,1], main=paste(lmain,n.names),xlim=xlim,ylim=ylim,xlab="", ylab="",type="l", lwd=1, cex=bcex)
	#axis(1, at=seq(0, length(dat.t[,1]), 5),tick=TRUE )  
	par(xpd=F)
	...
	# Tool for labeling cellular aspects, gfp.1, gfp.2, tritc, area
	legend(x=max(xlim)*.75, y=1.7, xpd=TRUE, inset=c(0,-.14), legend=c(
		if(!is.null(dat$c.dat[n.names, "CGRP"])){paste("CGRP","",round(dat$c.dat[n.names,"CGRP"],digits=0))},
		if(!is.null(dat$c.dat[n.names, "mean.gfp"])){paste("GFP","",round(dat$c.dat[n.names,"mean.gfp"],digits=0))},
		if(!is.null(dat$c.dat[n.names, "mean.gfp.1"])){paste("GFP.1","",round(dat$c.dat[n.names,"mean.gfp.1"],digits=0))},
		if(!is.null(dat$c.dat[n.names, "mean.gfp.2"])){paste("GFP.2","",round(dat$c.dat[n.names,"mean.gfp.2"],digits=0))},
	...
	if(Plotit.trace){
		lines(dat.t[,n.names]~dat.t[,1])
		points(dat.t[,n.names]~dat.t[,1], pch=16, cex=.5)
	}
...
rasterImage(img, 0, 1024, 1024, 0)
	...
	points(cell.x, cell.y, col=cols, pch=0, cex=2.4)
	text(cell.x, cell.y, labels=cell, col=cols, pos=2, cex=1)
	
	#points(cells.x, cells.y, col="white", pch=0, cex=bcex)
...
cell.zoom.1024<-function(dat, cell=NULL,img=NULL,  zoom=TRUE,zf=80, cols=NULL,lmain="", bcex=.8, labs=T, plot.new=T, cell.name=T)
{
if(plot.new){dev.new()}
require(png)
par(mar=c(0,0,1,0))
...
if(labs){
	if(!is.null(cell)){
		...
		text(cell.x, cell.y, labels=as.character(cell), col=cols, pos=2, cex=bcex)
	}
	else{
...
if(zoom==TRUE){
	...
	multi.pic.zoom(dat,cell,img, zf=zf) 
}
}
...
XYtrace.1024 <- function(dat, cell=NULL, img=NULL, cols=NULL, labs=F)
{
	...
	{	#selected name of cell
		...
		dev.set(dev.list()[1])
		PeakFunc5(dat,s.names, Plotit.both=F)
		dev.set(dev.list()[2])
		# If a cell is selected, that has already been selected, 
		# then remove that cell from the list
		if(length(intersect(i.names,s.names))==1){
		...
		if(length(i.names)>=1){dev.set(dev.list()[3]);LinesEvery.4(dat,m.names=i.names, plot.new=F, img=img, cols="black")}				
		dev.set(dev.list()[2])
...
cell.zoom.1024(dat,x.names,img)
