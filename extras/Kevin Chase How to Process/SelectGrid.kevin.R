...
SelectGrid <- function(tmp,x.names,pad=1.05,stain.name="tritc",title1="SelectRed",window.h=7,window.w=7,l.col="red")
{
	...
	zf[zf > x] <- x[zf > x]
	zf[zf > y] <- y[zf > y]
	zf[x+zf > 1024] <- 1024-x[x+zf > 1024]
	zf[y+zf > 1024] <- 1024-y[y+zf > 1024]
	
	img.left<-x-zf
	img.left[img.left < 1] <- 1
	img.right<-x+zf
	img.right[img.right > 1024] <- 1024
	img.top<-y-zf
	img.top[img.top < 1] <- 1
	img.bottom<-y+zf
	img.bottom[img.bottom > 1024] <- 1024

	img.bottom[img.top >= img.bottom & img.top < 1024] <- img.top[img.top >= img.bottom]+1
	img.right[img.left >= img.right & img.left < 1024] <- img.left[img.left >= img.right]+1

	img.top[img.top == 1024] <- 1023
	img.left[img.left == 1024] <- 1023
...
ROIreview <- function(tmp,x.names=NULL,pad=2,wh=7,hh=7,subset.n=NA)
{
	...
	for(x.names in subset.list)
	{
		...
		tmp$c.dat[names(r.names),"IB4.bin"] <- r.names
