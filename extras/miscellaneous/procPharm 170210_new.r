...
pharming_harvest <- function(main_dir="Y:/Mario Giacobassi/Manjus experiments", area_conversion=1.625, img_name_vec = NULL, image_question=T){
    ...
    for( i in 1:length(exp_dir) ){
        ...
        if(length(list.files(pattern="video_data.txt")) < 1){
            ...
            py_pharm$video_roi_extractor(video)
        }   
        require(data.table)
        ...
        if(length(list.files(pattern="time.info.txt")) < 1){
            py_pharm <- import('python_pharmer')
        ...
        for(j in 1:length(c_dat_names_update) ){
			value <- grep(c_dat_names_update[j], c_dat_names)
			if(length(value) > 0){
				cdnp_val[j] <- grep(c_dat_names_update[j], c_dat_names)
			}else{ cdnp_val[j] <- NA }
        }
	...
	if(!is.null(wrdef))
		{
			...
			Wr<-length(wr[,1])#complete and revise this section
			if(length(colnames(wr))<2){w.dat<-WrMultiplex(t.dat,wr,n=Wr)}
			else{w.dat <- MakeWr.docx(t.dat,wr)}
			tmp.rd$w.dat<-w.dat
		}
	levs<-setdiff(unique(as.character(tmp.rd$w.dat$wr1)),"")
...
pf.function_2<-function(dat, levs=NULL){
...
pf.summary<-function(dat, response.classes){
	
	pf_col<-menu(colnames(dat$bin))
	
	
	for(i in 1:length(response.classes)){
	
		response.types<-row.names(
			which(
				dat$bin[pf_col]==names(response.classes)[i]
			, arr.ind=T)
		)
		print(response.types)
		
		dat$bin[ names(response.classes)[i] ]<-0
		dat$bin[ response.types, names(response.classes)[i] ]<-1
	}
	return(dat)
}
...
SelectGrid <- function(tmp,x.names,pad=1.05,stain.name="area",title1="SelectRed",window.h=7,window.w=7,l.col="red",roi.img=NULL){
		...
		if(is.element(stain.name,c("tritc","gfp","dapi","cy5","tritc.immuno")))
		{
...
ROIreview <- function(tmp,x.names=NULL,pad=2,wh=7,hh=7,subset.n=500, roi.img=NULL){	
	...
	choices<-select.list(title="Score what?", choices=c("CGRP.GFP", "IB4.TRITC", "IB4.CY5", "NF200.TRITC", "Drops"), multiple=T)
...
LinesEvery.5 <- function(dat,m.names, img="img1",channel=NULL,pic.plot=TRUE,zf=NULL, t.type="mp.1", snr=NULL,lmain="",cols="black", levs=NULL, levs.cols="grey90", values=NULL,plot.new=T,sf=1,lw=1,bcex=1,p.ht=7,p.wd=10, lns=T, pts=F, underline=T,dat.n=NULL){
	...
	if(length(m.names) > 0)
		...
		## Tool for single color labeling
			...
			xinch(length(img))
			par(xpd=FALSE,mai=c(2,.5+(.5*length(img.l)),1,.8*length(values)), bty="l")
		...
		## Tool for adding Value info on right side of trace
			placement<-seq(0,length(values),.6)
			digits<-c(0,rep(4,length(values)))
			text(max(xseq)+xinch(placement[1:length(values)]), par("usr")[4]+yinch(.2), pos=4, values,cex=bcex*.95, srt=30)
		...
		tryCatch(legend(x=par("usr")[2]-xinch(1.2), y=par("usr")[3]-yinch(1.6), xpd=TRUE, inset=c(0,-.14), bty="n", cex=.7, legend=dat.name),error=function(e) NULL)
...
tcd<-function(dat, cells=NULL,img=dat$img1, l.img=c("img1"), yvar=FALSE, t.type="t.dat", plot.new=F, info=T, pts=T, lns=T, bcex=1, levs=NULL, klevs=NULL, sft=NULL, underline=T, zf=20, lw=2, sf=1, dat.name=NULL, view_func_description=F){
		...
		if(lines.flag==2){
			...
			tryCatch(
				LinesEvery.5(
					...
					img=l.img, lw=lw, t.type=t.type, col="black", lns=lns, levs=levs, bcex=1, underline=underline, dat.n=dat.name, zf=zf,sf=sf, values=values),error=function(e) print("You haven't stacked traces yet, yo."))
			lines.flag <- 0
		}
	...
	#P: Pick a group/cells to click through
		if(keyPressed=="P"){
		bringToTop(-1)
		print("Pick a Group of cells or a single cell to observe. If you Click cancel, all cells will be returned")
	...
	if(selection=='yes'){
		...
		save(gt.names,file=paste(save.names,'.Rdata',sep=''))
		gt.names<<-gt.names
	}else{
		gt.names<<-gt.names
        return(gt.names)
...
ImageFiller<-function(dat){
	...
	for(i in 1:8){
		...
		if(image.to.add==""){dat[[paste('img',i,sep='')]]<-NA
		}else{
...
Cell_Typer_2<-function(tmp_rd, edit_ct=T, UL_classify=T, GFP=T, cell_types=NULL){
    ...
    if(!is.null(cell_types)){
        ...
        if(length(large_cell_types_names)>0){
	...
	#identify red cells
	red_cells<-cellzand(tmp_rd$bin,"cy5.bin" ,1)
	...
	cap_cells<-cellzand(tmp_rd$bin,  
		grep("caps",names(tmp_rd$bin),ignore.case=T, value=T)[length(grep("caps",names(tmp_rd$bin),ignore.case=T, value=T))],
		1)
	...
	menth_cells<-cellzand(tmp_rd$bin, 
		grep("menth",names(tmp_rd$bin),ignore.case=T, value=T)[length(grep("menth",names(tmp_rd$bin),ignore.case=T, value=T))], 
		1)
	...
	menth_stat<-intersect(
		grep(".max", names(tmp_rd$scp), ignore.case=T, value=T),
		grep("menth",names(tmp_rd$scp),ignore.case=T, value=T)
	)
...
gif_pdf_maker<-function(dense=200,fps=2,file.name=NULL){
    require(magick)
    if(is.null(file.name)){
        file.name<-scan(n=1,what="character")
    }
    
    cat("Lets create a gif with this data, below are all pdfs in your experiment \n")
    cat(list.files(pattern="pdf"),sep="\n")
    pdf_imgs<-list.files(pattern="pdf")
    ...
    for(i in 1:imgs_for_gif){
        img_selection<-menu(list.files(pattern="pdf"),title=paste("Select image ",i))
...
Trace_select_grid<-function(dat, x.names, levs=select.list(names(dat$bin)), t.type="blc", preselect=T, l.col="red", window.w=10, window.h=10, title1="hi"){
    ...
    for(i in 1:xn){
        ...
        plot(dat[[t.type]][ levs_min:levs_max, x.names[i] ],type='l',lwd=3,xaxt='n',yaxt='n',col="white", ylim=c(-0.2,peak_max))
        dev.off()
		#print(i)
	}	
	end_time<-Sys.time()
...
PlotHeatMat <- function(mat,wt=14,ht=10,new.dev=T,title="TOPVIEW"){
	...
	par(xpd=T)
	points(par('usr')[1], par('usr')[3], col='white', cex=8, pch = 16)
...
TopView <- function(tmp, x.names=NULL, wt=7, ht=4, scale.var="mean.sm.sd", aux.var=c("diameter","IB45.bin","gfp5.bin"), img="img1", t.type="blc", title="TOPVIEW", stat_val='max'){
	#vet the vars
	...
	click.vals <- m.tot[ x.names, paste0(row.names(click.id),'.', stat_val) ]
	...
	while(xy.click$x > 0 | xy.click$y > 0)
	{
		...
		if(xy.click$y > 1)
		{
			...
			sval <- click.vals[x.names,paste0(sort.trt,'.',stat_val)]
