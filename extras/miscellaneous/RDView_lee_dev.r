...
ScoreLev <- function(wt,bin=NULL,ignore=NULL,col1=NULL,main.lab=NULL,bin.cat=3,rd.name="",wh=14,hh=8, tmp_rd=tmp)
{
	#score 1 region
	...
	while(not.done){
		if(redraw==T){
			...
			ygap <- ylim[2]-ylim[1]
			colMean <- function(x){
				xcol <- apply(col2rgb(x), 1, mean)
				return(rgb(xcol[1],xcol[2],xcol[3],alpha=255,maxColor=255))
			}
			...
			if(first.time==TRUE){
				first.time <- FALSE
				#TOOL FOR AUTOMATING THE WINDOWS WIDTH
				w.dat_time <- range(tmp_rd$w.dat$Time[tmp_rd$w.dat$wr1 == main.lab])
				window_width_size <- w.dat_time[2] - w.dat_time[1] 
				wh <- window_width_size * 3.5
				dev.new(width=wh, height=hh,family="mono",canvas="black") 
				par(fg="white",col.axis="white",col.lab="grey",col.main="grey",mar=c(3,2,2,1))
				xlim <- c( (xsec[1]-xsec[2]), max(xsec) )
				print(xlim)
				plot(xsec, wt[,2], xlim=xlim, ylim=ylim, xlab="Time (seconds)", ylab="Ratio", type="n", xaxt="n")	
				smax <- max( strwidth( paste( grp.tab[,"col.text"]," "), family="mono") )
				xlim = range(xsec)
				wt[,1] <- wt[,1]*60
				tmp.png <- BackgroundRaster(wt,hh,wh,tcol50,xlim,ylim)
				wt[,1] <- wt[,1]/60						
			}
			#INITIAL PLOTTING
			#xlim <- c(min(xsec)-xinch(3), max(xsec))
			#xlim <- c((xsec[1]-xsec[2])-xinch(3),max(xsec))
			xlim <- c( ((xsec[1]-xsec[2]) - smax)-xinch(2) , max(xsec) )
			print(xlim)
			plot(xsec, wt[,2], ylim=ylim, xlim=xlim, xlab="Time (seconds)", ylab="Ratio", type="n", xaxt="n")		
			xlim_new = range(xsec)
			rasterImage(tmp.png,xlim_new[1],ylim[1],xlim_new[2],ylim[2])
			
			#TOOL TO COLOR TRACES
			require(RColorBrewer)
			...
			for(i in dimnames(ret.group)[[2]]){
				lines(xsec,t(ret.group[,i]),col="white",lwd=5)
				lines(xsec,t(ret.group[,i]),col=cols[i],lwd=4)
			}
			#X AXIS LABEL
			axis(1,at=xsec, labels=FALSE)
			text(x=xsec, par("usr")[3]-.03, labels=orig.time, pos = 1, xpd = TRUE, col="white", cex=.5)
			#MAIN LABEL
			...
			#LEFT SIDE POINTS LABEL

			points(grp.tab[,"m1x"]-xinch(1.75), grp.tab[,"m1y"], pch="+", cex=2)
			points(grp.tab[,"m1x"]-xinch(1.50),grp.tab[,"m1y"], pch="-", cex=2)
			o.x <- rep(max(xsec),7)
			o.y <- seq(ylim[2]*.80,ylim[2],length.out=9)[2:8]
			points(o.x, o.y, pch=16,cex=2)
			text(o.x,o.y,c("Top","Done","Back","All","1s","0s","png.out"),pos=2)
			loc.x <- c(grp.tab[,"m1x"]-xinch(1.75), grp.tab[,"m1x"]-xinch(1.5), grp.tab[,"m1x"], xsec, o.x)
...
#	if(!is.element("grps",names(tmp))){tmp$grps <- tmp$bin[,tlevs];tmp$grps[] <- NA}
	...
	while(sel.i != 0)
	{
		...
		if(sel.i > 0)
		{
			...
			wt.sl <- ScoreLev(wt,tmp$bin[c.i,tlevs[sel.i]],tmp$w.dat[r.i,"ignore"],col50,main.lab=tlevs[sel.i],rd.name=rd.name,wh=wh,hh=hh,tmp_rd=tmp)
