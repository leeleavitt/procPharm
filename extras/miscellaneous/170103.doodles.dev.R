...
ScoreLev <- function(wt,bin=NULL,ignore=NULL,col1=NULL,main.lab=NULL,bin.cat=3,rd.name="",wh=14,hh=8)
{
	...
	graphics.off()
	#WHERE DOES WINDOW START
	orig.time<-round(wt[,1],digits=2)
	
	if(wt[1,1] > 0){
		wt[,1] <- wt[,1]-wt[1,1]
	}
	#NO BIN DATAFRAME THEN  
	if(is.null(bin)){
		bin <- rep(0,ncol(wt)-1)
	}
	...
	tcol50 <- col1
	colfunc <- function(x){
		return(rgb(red=x[1],green=x[2],blue=x[3],alpha=64,maxColorValue=255))
	}
	...
	#INITIALIZE VALUES FOR INTERACTIVE FUNCTIONALITY
	my.list <- list()
	my.list[[1]] <- sub.names
	list.i <- 1
	first.time = TRUE
	...
	while(not.done){
		if(redraw==T){
			...
			for(i in dimnames(ret.group)[[2]]){
				lines(xsec,t(ret.group[,i]),col="white",lwd=5);lines(xsec,t(ret.group[,i]),col=cols[i],lwd=4)
			}
		...
		#positive selected for a group
		if(click.i > ng & click.i <= ng*2) {
		...
		#toggle ignore points
		if(click.i > ng*3 & click.i <= (ng*3+length(ignore))) {
			ig.i <- click.i-ng*3 
			if(ignore[ig.i] == 0){
				points(loc.x[click.i],loc.y[click.i],pch=17,col="red",cex=1.1)
			}else{
				points(loc.x[click.i],loc.y[click.i],pch=17,col="black",cex=1.1)
				points(loc.x[click.i],loc.y[click.i],pch=2,col="grey",cex=.1)
			}
			...
			#png.out things are getting messy
			if(o.i==7) {
				...
				for(i in dimnames(ret.group)[[2]]){
					lines(xsec,t(ret.group[,i]),col="black",lwd=6)
					lines(xsec,t(ret.group[,i]),col=grp.tab[i,"color"],lwd=5)
				}
				axis(1,at=xsec, labels=FALSE)
...
RDView <- function(tmp,wr.i="wr1",rd.name=NULL,c.i=NULL,rscale=F,wh=14,hh=8){k
