...
SelectGrid <- function(tmp,x.names,pad=1.05,stain.name="tritc",title1="SelectRed",window.h=7,window.w=7,l.col="red")
{
	...
	img.yellow <- imgs[which.max(img.rgb[,"r"]+img.rgb[,"g"])]
...
LinesEvery.5 <- function(dat,m.names, img=dat$img1,pic.plot=TRUE, multi.pic=T,zf=NULL, t.type=FALSE, snr=NULL,lmain="",cols=NULL, levs=NULL, levs.cols="grey90",m.order=NULL,rtag=NULL,rtag2=NULL,rtag3=NULL,plot.new=T,sf=1,lw=.9,bcex=.6,p.ht=7,p.wd=10)
{
	require(png)
	#if(blc){t.dat<-dat$blc}
	if(t.type){t.type<-menu(names(dat));t.dat<-dat[[t.type]]}# if trace type is empty select the data, you would like your trace to be
...
### Picture Plotting!		
	...
	if(pic.plot==TRUE & length(m.names)<=10){
		...
		par(mar=c(0,0,0,0))
		plot(0,0,xlim=c(0,6), ylim=c(0,6), xaxs="i",yaxs="i", xaxt='n', yaxt='n')
		tmp.img<-multi.pic.zoom.2(dat, m.names,img=img.p, labs=T)
		dev.set(bob) # FUCK THIS!
		rasterImage(tmp.img, 0,0,6,6)
	}
	}
...
graphics.off()
	...
	if(length(m.names)>subset.n){
		...
		LinesEvery.5(dat, sample(row.names(dat$c.dat)[1:5]), plot.new=F)
	...
	if(continue=="yes"){
		while(i!=length(s.names)+1){
			i<-scan(n=1)
			if(i>length(s.names)| i==0){i<-length(s.names)+1}else{
			LinesEvery.5(dat,sample(names(which(info==i, arr.ind=T)))[1:15], img, pic.plot=T, sf=.3, lmain=i,m.order="area")}
			#multi.pic.zoom(dat, names(which(info==i, arr.ind=T)), img, plot.new=T)
		}
	}
	}else{LinesEvery.5(dat, m.names,img)}
...
PeakFunc6 <- function(dat,n.names,select.trace="t.dat",Plotit.trace=T,Plotit.both=F, info=T,lmain=NULL, bcex=.7, yvar=T, ylim.max=NULL)
{
	if(class(select.trace)=="character"){
		dat.select<-select.trace
		dat.t<-dat[[dat.select]]
...
Trace.Click<-function(dat, cells=NULL,img=dat$img1, yvar=FALSE, select.trace="t.dat")
{
    graphics.off()
    dev.new(width=14,height=4)    
    dev.new(width=10,height=6)  
    ...
    while(click.i!=7)
    {
        cell.pick <- c.names[cell.i]
        dev.set(dev.list()[1])
        p1 <- PeakFunc6(dat,cell.pick, select.trace=select.trace,yvar=yvar )
        p1.par<-par()
		if(lines.flag==2){
			dev.set(dev.list()[3])
		...
		if(lines.flag==1){
			dev.set(dev.list()[2]);
			LinesEvery.5(dat,g.names,plot.new=F, img=img)
			lines.flag <- 0
		}
		...
		if(click.i==6){
			select.trace<-select.list(names(dat))
		}
		if(click.i==7){graphics.off()}
}
print(g.names)}
...
XYtrace.2048.2 <- function(dat, cell=NULL, img=NULL, cols=NULL, labs=F, zf=60)
{
	x.coor<-grep("\\.x",names(dat$c.dat), value=T, ignore.case=T)
	y.coor<-grep("\\.y",names(dat$c.dat), value=T, ignore.case=T)
	area<-grep("area",names(dat$c.dat), value=T, ignore.case=T)
	...
	dev.set(dev.list()[2])
	par(mar=c(0,0,0,0))
	plot(0, 0, xlim=c(0,2048),ylim=c(2048,0),xaxs="i", yaxs="i",col=cols,pch=".")
	
	if(is.null(img)){img<-dat$img1}
	if(!is.null(img)){rasterImage(img, 0, 2048, 2048, 0);points(cell.coor[,1],cell.coor[,2],col=cols,pch=3,cex=1)}
	else{
	points(cell.coor[,1],cell.coor[,2], col=cols, cex=dat$c.dat[,area]/200)
	points(cell.coor[,1],cell.coor[,2],col=cols, pch=4)}
...
col.row<-ceiling(sqrt(length(m.names)))
	...
	for(i in 1:length(m.names)){		
		...
		right<-x+zf
		if(right>=2048-zf){left=2048-zf;right=2048}
					
		y<-dat$c.dat[m.names[i],"center.y"]
		top<-y-zf
		if(top<=20){top=0; bottom=zf}
		bottom<-y+zf
		if(bottom>=2048-zf){top=2048-zf;bottom=2048}
...
for(i in 1:length(m.names)){		
	...
	if(labs){
		...
		for(j in 1:length(label.names)){
			text(16.5, label.y.location[j], labels=round(dat$c.dat[m.names[i],label.names[j]],digits=5), col=label.cols[j])
		}
	}
}	
	dev.off()png::readPNG
